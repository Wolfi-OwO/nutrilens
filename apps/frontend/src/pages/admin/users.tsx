...
export default function AdminUsersPage() {
  ...
  const applyChange = (target: PublicUser, body: { role?: UserRole; status?: 'active' | 'suspended' }) => {
    setActionError(null)
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Search, filter, and manage every account.</p>
      </div>
                ...
                {users.data.users.map((target) => {
                  ...
                  return (
                    <TableRow key={target.id}>
                      <TableCell>
                        <p className="font-medium text-foreground">{target.displayName}</p>
                      ...
                      <TableCell>
                        <select
                          ...
                          onChange={(e) => {
                            applyChange(target, { role: e.target.value as UserRole })
                          }}
                          className={selectClassName}
                      ...
                      <TableCell className="text-right">
                        {target.status !== 'deleted' && (
                          <Button
                            ...
                            variant="outline"
                            size="sm"
                            disabled={changeRoleStatus.isPending || (isSelf && target.status === 'active')}
                            title={isSelf && target.status === 'active' ? "You can't suspend your own account." : undefined}
                            onClick={() => {
                              applyChange(target, { status: target.status === 'active' ? 'suspended' : 'active' })
                            }}
                          >
                            {target.status === 'active' ? 'Suspend' : 'Reactivate'}
                          </Button>
                        )}
                      </TableCell>
                    </TableRow>
