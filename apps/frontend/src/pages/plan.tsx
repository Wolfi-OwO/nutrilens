...
function outOfBounds(values: TargetsForm): string | null {
  if (values.dailyCalorieTarget < CALORIE_BOUNDS.min || values.dailyCalorieTarget > CALORIE_BOUNDS.max) {
...
const STATS: { key: keyof TargetsForm; label: string; icon: LucideIcon; className: string }[] = [
  { key: 'dailyCalorieTarget', label: 'Daily calories', icon: Flame, className: 'bg-primary/15 text-primary' },
  { key: 'proteinTargetGrams', label: 'Protein', icon: Beef, className: 'bg-chart-protein/15 text-chart-protein' },
  { key: 'carbTargetGrams', label: 'Carbs', icon: Wheat, className: 'bg-chart-carb/15 text-chart-carb' },
  { key: 'fatTargetGrams', label: 'Fat', icon: Droplet, className: 'bg-chart-fat/15 text-chart-fat' },
]
      ...
      <CardContent className="flex flex-col gap-6 pt-6">
        <div className="flex items-baseline justify-between">
          <p className="text-sm text-muted-foreground">
            Goal: <span className="font-medium text-foreground">{formatGoal(plan.goal)}</span> · active
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {STATS.map((stat) => (
              <div key={stat.key} className="flex flex-col gap-1.5">
                <Label htmlFor={stat.key} className="flex items-center gap-1.5 text-muted-foreground">
                  <span className={cn('flex h-6 w-6 items-center justify-center rounded-md', stat.className)}>
                    <stat.icon size={13} strokeWidth={2.25} />
                  </span>
                  {stat.label}
                </Label>
                <Input id={stat.key} type="number" inputMode="decimal" {...register(stat.key)} />
              </div>
            ))}
          </div>
        ...
        <form onSubmit={(e) => void handleSubmit(onSubmit)(e)} className="flex flex-col gap-5" noValidate>
              ...
              <label key={option.value}>
                ...
                <span
                  className={cn(
                    ...
                    goal === option.value && 'border-primary bg-primary text-primary-foreground hover:bg-primary/90',
                  )}
                >
                  {option.label}
                </span>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Input id="carbTargetGrams" type="number" inputMode="decimal" {...register('carbTargetGrams')} />
            </div>
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="fatTargetGrams">Fat (g)</Label>
              <Input id="fatTargetGrams" type="number" inputMode="decimal" {...register('fatTargetGrams')} />
            </div>
          </div>
