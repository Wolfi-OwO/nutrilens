  ...
  const handlePhotoSelected = async (file: File) => {
    ...
    try {
      ...
      if (result.available && result.predictions && result.predictions.length > 0) {
        ...
        setAiUnavailableReason(null)
        reset({ items: [{ ...EMPTY_ITEM, foodName: formatLabel(result.predictions[0]?.label ?? '') }] })
      } else {
        ...
        setPredictions(null)
        setPredictionsLowConfidence(false)
        setAiUnavailableReason("Couldn't identify this photo automatically — enter what you ate below.")
        reset({ items: [EMPTY_ITEM] })
      }
      setStage('reviewing')
    } catch {
              ...
              <CardContent className="pt-6">
                ...
                {predictionsLowConfidence && (
                  <p className="mb-3 mt-1 text-sm text-muted-foreground">
                    Not very confident about this one — double check before confirming.
                  </p>
                )}
                <ul className={cn('flex flex-wrap gap-2', !predictionsLowConfidence && 'mt-3')}>
          ...
          <Card>
            <CardContent className="flex flex-col gap-5 pt-6">
              {fields.map((field, index) => (
                <div key={field.id} className="flex flex-col gap-3 border-b border-border pb-5 last:border-0 last:pb-0">
                  ...
                  {errors.items?.[index]?.foodName && (
                    <p className="text-sm text-destructive">{errors.items[index]?.foodName?.message}</p>
                  )}
                  ...
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
                    ...
                    <div className="flex flex-col gap-1.5">
                      <Label htmlFor={`items.${index}.calories`}>Calories</Label>
                      <Input
                    ...
                    <div className="flex flex-col gap-1.5">
                      <Label htmlFor={`items.${index}.proteinGrams`}>Protein (g)</Label>
                      <Input
                    ...
                    <div className="flex flex-col gap-1.5">
                      <Label htmlFor={`items.${index}.carbGrams`}>Carbs (g)</Label>
                      <Input
                    ...
                    <div className="flex flex-col gap-1.5">
                      <Label htmlFor={`items.${index}.fatGrams`}>Fat (g)</Label>
                      <Input
          ...
          {submitError && (
            <p role="alert" className="rounded-lg bg-destructive/10 px-3 py-2 text-sm text-destructive">
              ...
              {needsPlan && (
                <>
                  {' '}
                  <Link to="/plan" className="font-medium underline underline-offset-2">
                    Set up a plan
                  </Link>
                  .
                </>
