...
export default function ProgressPage() {
  ...
  const calorieTrend = useMemo(() => {
    ...
    return days.map((day) => ({ day, label: formatShortDate(day), calories: totals.get(day) ?? 0 }))
  }, [mealLogs.data])
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Weight and calorie trend history.</p>
      </div>
            ...
            <CardContent>
              {mealLogs.data && mealLogs.data.length === 0 ? (
                <p className="py-6 text-center text-sm text-muted-foreground">
                  No meals logged yet — your calorie trend will show up here once you do.
                </p>
              ) : (
                <div className="h-56 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={calorieTrend} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            ...
            <CardContent className="flex flex-col gap-4">
              {weightTrend.length === 0 ? (
                ...
                <p className="py-2 text-center text-sm text-muted-foreground">
                  ...
                  logged on {weightTrend[0].label} — one more weigh-in will start a trend line.
                </p>
              ) : (
                <>
                  <div className="h-56 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={weightTrend} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" vertical={false} />
                        <XAxis
                          dataKey="label"
                          tick={{ fontSize: 12, fill: 'var(--muted-foreground)' }}
                  ...
                  <p className="border-t border-border pt-3 text-sm text-muted-foreground">
                    <span className="font-mono font-semibold tabular-nums text-foreground">
                      {(weightTrend[weightTrend.length - 1].kg - weightTrend[0].kg).toFixed(1)} kg
                    </span>{' '}
...
function LogWeightForm() {
  ...
  return (
    <form onSubmit={(e) => void onSubmit(e)} className="flex flex-col gap-2 border-t border-border pt-4">
      ...
      <div className="flex gap-2">
        <Input
          ...
          className="max-w-32"
        />
        <Button type="submit" variant="outline" disabled={createWeightEntry.isPending || !weight}>
